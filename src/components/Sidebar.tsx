...
import { LayoutDashboard, BarChart3, Store, Package } from 'lucide-react'
...
export function Sidebar() {
  return (
    <aside className="w-64 shrink-0 h-screen sticky top-0 flex flex-col bg-white sidebar-panel">
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) =>
                clsx(
                  'nav-item group flex items-center gap-3 px-4 py-4 rounded-xl text-base font-medium',
                  isActive
            ...
            >
              <Icon size={20} strokeWidth={2} className="transition-transform duration-200 group-hover:scale-125 group-hover:rotate-[-6deg]" />
              {item.label}
            </NavLink>
          )
        })}
      ...
      <div className="px-6 py-4 border-t border-[var(--color-border)] text-[11px] text-[var(--color-muted)]">
        Data synced 5 am and 5 pm daily. 
      </div>
    </aside>
  )
}